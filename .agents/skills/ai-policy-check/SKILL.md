...
- **Commit**: the message ends with `Assisted-by: AGENT_NAME:MODEL_VERSION`, naming the
  exact model used, e.g. `Assisted-by: GitHub Copilot Chat:gpt-5.6-luna`. Do not add a
  `Co-authored-by:` trailer naming an AI. `AI_POLICY.md` accepts a `Co-authored-by:`
  trailer that a tool added automatically as valid disclosure: in your own commit,
  replace it with the preferred `Assisted-by:` form; in someone else's commit, leave it
  alone.
